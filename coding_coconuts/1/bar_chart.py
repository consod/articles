...
def render(app, df, worker_names):
    ...
    def update_bar_chart(all_equipment):
        long_format_df = pd.melt(df, id_vars=['Equipment'], value_vars=worker_names, var_name='Name', value_name='Nr of times worked')
        filtered_data = long_format_df.query("Equipment in @all_equipment")
        fig = px.bar(filtered_data,x="Equipment", y="Nr of times worked", color="Name", barmode="group")
        fig.update_layout(
